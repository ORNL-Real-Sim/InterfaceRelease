...
% %             try
                ...
                while round(msgProcessed) < round(totalMsgSize)
                    ...
                    if iMsgTypeRecv == obj.vehMsgIdentifer
						...
						% translate to Toyota conventions
                        if obj.VehicleMessageFieldDef.('signalLightColor')
                            % originally signal color is transmitted based on
                            % RealSim signalLightColor id as followings:
                            % red = 1, amber = 2, green = 3, red / amber = 4, amber flashing = 5, off = 6, other = 0
                            clrVissim = double(VehDataBus.('signalLightColor'));
                            
                            % convert to Toyota id
                            if round(clrVissim) == 3
                                VehDataBus.signalLightColor = 0;
                            elseif round(clrVissim) == 2
                                VehDataBus.signalLightColor = 1;
                            elseif round(clrVissim) == 1
                                VehDataBus.signalLightColor = 2;
                            else
                                VehDataBus.signalLightColor = -1;
                            end
                        end
