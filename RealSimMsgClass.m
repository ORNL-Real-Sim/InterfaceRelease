classdef RealSimMsgClass < handle
    ...
    properties
        VehicleMessageFieldDefInputVec = zeros(1, 28); % Vector of 0,1 to select which message field will be transmitted
    ...
    properties(Access = protected)
        ...
        VehicleMessageFieldDef = struct('id', 0, 'type', 0, 'speed', 0, 'acceleration', 0, ...
...
%             coder.allowpcode('plain')
    ...
    methods(Access = public)
        ...
        % convert VehicleMessageFieldDefInputVec to VehicleMessageFieldDef
            VehMsgAllList = {'id', 'type', 'speed', 'acceleration', ...
...
%             VehData = struct('id', uint8(zeros(50,1)), 'idLength', 0, 'type', uint8(zeros(50,1)), 'typeLength', 0, ...
%                     'speed', 0, 'acceleration', 0, 'positionX', 0, 'positionY', 0, 'positionZ', 0, ...
%                     'heading', 0, 'color', 0, 'linkId', uint8(zeros(50,1)), 'linkIdLength', 0, 'laneId', 0, ...
%                     'distanceTravel', 0, 'speedDesired', 0, 'accelerationDesired', 0, ...
%                     'hasPrecedingVehicle', 0, 'precedingVehicleId', uint8(zeros(50,1)), 'precedingVehicleIdLength', 0, 'precedingVehicleDistance', 0, 'precedingVehicleSpeed', 0, ...
%                     'signalLightId', 0, 'signalLightDistance', 0, 'signalLightColor', 0, ...
%                     'speedLimit', 0, 'speedLimitNext', 0, 'speedLimitChangeDistance', 0, ...
%                     'linkIdNext', uint8(zeros(50,1)), 'linkIdNextLength', 0, 'grade', 0, 'activeLaneChange', 0);
            iByte = 1;
            ...
            if obj.VehicleMessageFieldDef.('id')
                ...
                VehData.('id') = uint8Arr; %str(1:strLen);
                VehData.('idLength') = strLen;
            end
            if obj.VehicleMessageFieldDef.('type')
                [str, strLen, iByte, uint8Arr] = obj.depackString(ByteData, iByte);
                VehData.('type') = uint8Arr; %str(1:strLen);
            ...
            if obj.VehicleMessageFieldDef.('linkId')
                [str, strLen, iByte, uint8Arr] = obj.depackString(ByteData, iByte);
                VehData.('linkId') = uint8Arr; %str(1:strLen);
            ...
            if obj.VehicleMessageFieldDef.('precedingVehicleId')
                [str, strLen, iByte, uint8Arr] = obj.depackString(ByteData, iByte);
                VehData.('precedingVehicleId') = uint8Arr; %str(1:strLen);
            ...
            if obj.VehicleMessageFieldDef.('signalLightId')
                VehData.('signalLightId') = double(typecast(ByteData(iByte:iByte+3),'int32'));
                iByte = iByte+4;
            end
...
%                 str = [str, ByteData(iByte)];
        ...
        function [ByteData, nMsgSize] = packVehData(obj, simState, t, ByteData, VehData)
            ...
            nMsgSize = round( obj.msgHeaderSize + obj.msgEachHeaderSize  ...
                + obj.VehicleMessageFieldDef.('id')*(1 + numel(VehData.('id'))) ...
                + obj.VehicleMessageFieldDef.('type')*(1 + numel(VehData.('type'))) ...
                ...
                + obj.VehicleMessageFieldDef.('linkId')*(1 + numel(VehData.('linkId'))) ... % linkId
                ...
                + obj.VehicleMessageFieldDef.('precedingVehicleId')*(1 + numel(VehData.('precedingVehicleId'))) ... % precedingVehicleId
                ...
                + obj.VehicleMessageFieldDef.('signalLightId')*(4) ... % signalLightId
                ...
                + obj.VehicleMessageFieldDef.('linkIdNext')*(1 + numel(VehData.('linkIdNext'))) ... % linkIdNext
            ...
            if obj.VehicleMessageFieldDef.('signalLightId')
                % signalLightId
                tempInt32 = typecast(int32(VehData.('signalLightId')), 'uint8');
                ByteData(iByte:iByte+3) = tempInt32;
                iByte = iByte + 4;
            end
